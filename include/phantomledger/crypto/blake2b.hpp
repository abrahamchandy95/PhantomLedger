...
inline constexpr std::size_t blockBytes = 128;
inline constexpr std::size_t maxDigestBytes = 64;
inline constexpr std::size_t maxKeyBytes = 64;
inline constexpr std::size_t rounds = 12;
...
inline constexpr std::array<std::uint64_t, 8> initialVector{
    0x6A09E667F3BCC908ULL, 0xBB67AE8584CAA73BULL, 0x3C6EF372FE94F82BULL,
    0xA54FF53A5F1D36F1ULL, 0x510E527FADE682D1ULL, 0x9B05688C2B3E6C1FULL,
    0x1F83D9ABFB41BD6BULL, 0x5BE0CD19137E2179ULL,
};

inline constexpr std::array<std::array<std::uint8_t, 16>, rounds> sigma{{
...
[[nodiscard]] inline std::uint64_t rotr64(std::uint64_t value,
                                          int count) noexcept {
...
inline void storeU64Le(void *dest, std::uint64_t value) noexcept {
  if constexpr (std::endian::native == std::endian::big) {
    value = std::byteswap(value);
  }
...
inline void addCount(State &state, std::uint64_t amount) noexcept {
...
inline void mixG(std::array<std::uint64_t, 16> &work, int a, int b, int c,
                 int d, std::uint64_t x, std::uint64_t y) noexcept {
  work[a] += work[b] + x;
...
inline void compress(State &state, const std::uint8_t *block) noexcept {
  ...
  for (int round = 0; round < static_cast<int>(rounds); ++round) {
...
[[nodiscard]] inline bool start(State &state, std::size_t outSize,
                                const void *key = nullptr,
                                std::size_t keySize = 0) noexcept {
  if (outSize == 0 || outSize > maxDigestBytes) {
    return false;
  }
  ...
  if (keySize > 0) {
    ...
    state.bufSize = blockBytes;
  }

  return true;
}

[[nodiscard]] inline bool absorb(State &state, const void *data,
  ...
  while (size > 0) {
    if (state.bufSize == blockBytes) {
      addCount(state, blockBytes);
      compress(state, state.buf.data());
      state.bufSize = 0;
    }

    const auto writable = std::min(blockBytes - state.bufSize, size);
...
[[nodiscard]] inline bool finish(State &state, void *output) noexcept {
...
[[nodiscard]] inline bool digest(const void *data, std::size_t size,
                                 void *output, std::size_t outSize,
                                 const void *key = nullptr,
                                 std::size_t keySize = 0) noexcept {
  detail::State state{};
  ...
  if (!detail::absorb(state, data, size)) {
    detail::secureZero(&state, sizeof(state));
    return false;
  }
