...
void testBuildCdfSingle() {
  const auto cdf = distributions::buildCdf(std::vector<double>{42.0});
...
void testBuildCdfErrors() {
  bool caught = false;
  try {
    (void)distributions::buildCdf(std::vector<double>{});
  ...
  caught = false;
  try {
    (void)distributions::buildCdf(std::vector<double>{1.0, -1.0});
  } catch (const std::invalid_argument &) {
    caught = true;
  }
  PL_CHECK(caught);
  ...
  caught = false;
  try {
    (void)distributions::buildCdf(std::vector<double>{0.0, 0.0});
  } catch (const std::invalid_argument &) {
    caught = true;
  }
  PL_CHECK(caught);
...
void testSampleIndexValues() {
  ...
  for (const auto &c : cases) {
    PL_CHECK_EQ(distributions::sampleIndex(cdf, c.u), c.expected);
  }
...
void testSampleIndexEdges() {
  ...
  PL_CHECK_EQ(distributions::sampleIndex(cdf, 0.25), 1U);
  PL_CHECK_EQ(distributions::sampleIndex(cdf, 0.50), 2U);
  PL_CHECK_EQ(distributions::sampleIndex(cdf, 0.0), 0U);
  PL_CHECK_EQ(distributions::sampleIndex(cdf, -1.0), 0U);
  PL_CHECK_EQ(distributions::sampleIndex(cdf, 2.0), 3U);
...
void testSampleIndexErrors() {
  bool caught = false;
  try {
    (void)distributions::sampleIndex(std::vector<double>{}, 0.5);
  } catch (const std::invalid_argument &) {
    caught = true;
  }
  PL_CHECK(caught);
